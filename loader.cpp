 ...
 * Copyright (C) 2013 Martin Ribelotta (martinribelott@gmail.com) Licensed under
 * GNU GPL v2 or later Modified by Jim Huang (jserv.tw@gmail.com)
 */
...
static int relocateSymbol(Elf32_Addr relAddr, int type, Elf32_Addr symAddr,
  ...
  case R_XTENSA_SLOT0_OP: {
    ...
    *from = v;
    ...
    /* *** Format: BRI8  *** */
    /* *** BALL, BANY, BBC, BBCI, BBCI.L, BBS,  BBSI, BBSI.L, BEQ, BGE,  BGEU,
     * BLT, BLTU, BNALL, BNE,  BNONE, LOOP,  *** */
...
static ELFLoaderSection_t *findSection(ELFLoaderContext_t *ctx, int index) {
  for (ELFLoaderSection_t *section = ctx->section; section != NULL;
...
static Elf32_Addr findSymAddr(ELFLoaderContext_t *ctx, Elf32_Sym *sym,
                              const char *sName) {
...
static int relocateSection(ELFLoaderContext_t *ctx, ELFLoaderSection_t *s) {
  ...
  MSG("  Offset   Sym  Type                      relAddr  symAddr  defValue    "
      "                Name + addend");
  for (size_t relCount = 0; relCount < relEntries; relCount++) {
    LOADER_GETDATA(ctx, sectHdr.sh_offset + relCount * (sizeof(rel)), &rel,
                   sizeof(rel))
    Elf32_Sym sym;
    ...
    Elf32_Addr relAddr =
        ((Elf32_Addr)s->data) + rel.r_offset; // data to be updated adress
    readSymbol(ctx, symEntry, &sym, name, sizeof(name));
    Elf32_Addr symAddr =
        findSymAddr(ctx, &sym, name) + rel.r_addend; // target symbol adress
    uint32_t from = 0;
    uint32_t to = 0;
    if (relType == R_XTENSA_NONE || relType == R_XTENSA_ASM_EXPAND) {
      //            MSG("  %08X %04X %04X %-20s %08X          %08X %s + %X",
      ...
      MSG("  %08X %04X %04X %-20s %08X %08X %08X                    %s + %X",
          rel.r_offset, symEntry, relType, type2String(relType), relAddr,
          symAddr, sym.st_value, name, rel.r_addend);
      r = -1;
    } else if (relocateSymbol(relAddr, relType, symAddr, sym.st_value, &from,
                              &to) != 0) {
      ERR("  %08X %04X %04X %-20s %08X %08X %08X %08X->%08X %s + %X",
          rel.r_offset, symEntry, relType, type2String(relType), relAddr,
          symAddr, sym.st_value, from, to, name, rel.r_addend);
      r = -1;
    } else {
...
ELFLoaderContext_t *elfLoaderInitLoadAndRelocate(LOADER_FD_T fd,
                                                 const ELFLoaderEnv_t *env) {
  MSG("ENV:");
  ...
  ELFLoaderContext_t *ctx =
      (ELFLoaderContext_t *)malloc(sizeof(ELFLoaderContext_t));
  assert(ctx);
...

  {
    ...
    ".strtab": segment points to the actual string names used by the symbol
    table
    */
    ...
    for (int n = 1; n < ctx->e_shnum; n++) {
      ...
      if (sectHdr.sh_flags & SHF_ALLOC) {
        if (!sectHdr.sh_size) {
          ...
          ELFLoaderSection_t *section =
              (ELFLoaderSection_t *)malloc(sizeof(ELFLoaderSection_t));
          assert(section);
        ...
        if (sectHdr.sh_info >= n) {
          ERR("Rela section: bad linked section (%i:%s -> %i)", n, name,
              sectHdr.sh_info);
          goto err;
        }
        ELFLoaderSection_t *section = findSection(ctx, sectHdr.sh_info);
        if (section == NULL) {
          MSG("  section %2d: %-15s -> %2d: ignoring", n, name,
              sectHdr.sh_info);
        } else {
          section->relSecIdx = n;
...
int elfLoaderSetFunc(ELFLoaderContext_t *ctx, const char *funcname) {
  ...
  for (int symCount = 0; symCount < ctx->symtab_count; symCount++) {
    ...
    if (strcmp(name, funcname) == 0) {
      ...
      if (symAddr == 0xffffffff) {
        MSG("  %04X %-30s %04X %08X %04X ????????", symCount, name,
            sym.st_shndx, sym.st_value, sym.st_size);
      } else {
        ctx->exec = (void *)symAddr;
        MSG("  %04X %-30s %04X %08X %04X %08X", symCount, name, sym.st_shndx,
            sym.st_value, sym.st_size, symAddr);
      }
    } else {
      MSG("  %04X %-30s %04X %08X %04X", symCount, name, sym.st_shndx,
...
int elfLoader(LOADER_FD_T fd, const ELFLoaderEnv_t *env, char *funcname,
              int arg) {
